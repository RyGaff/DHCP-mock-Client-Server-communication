...
msg_t populateMessage(FILE *);

void printOutput(msg_t, char *);

char* getHardware(uint);

char* getOp(uint);

bool checkCookie(FILE *);

void printOptions(uint8_t);

void printTime(uint32_t);

void leaseTime(uint8_t *, uint32_t, int );

void servIden(uint8_t *, uint32_t, int);

void options(uint8_t *, bool);

void recvClient(uint8_t *, int socketfd, struct sockaddr_in server, ssize_t bytes);

void update_id_req(uint8_t *);
#endif
